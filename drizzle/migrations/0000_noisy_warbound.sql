...
CREATE TABLE `column_transition` (
	...
	FOREIGN KEY (`from_column_id`) REFERENCES `board_column`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`to_column_id`) REFERENCES `board_column`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
